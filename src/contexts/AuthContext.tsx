...
interface AuthContextType {
    ...
    signUp: (email: string, password: string, fullName: string, clinicName: string, selectedPlan: string, cardToken?: string | null, paymentProvider?: string, referralCode?: string, turnstileToken?: string, attribution?: Record<string, unknown>, phone?: string) => Promise<{ error: Error | null }>
...
export function AuthProvider({ children }: { children: ReactNode }) {
    ...
    const signUp = async (email: string, password: string, fullName: string, clinicName: string, selectedPlan: string, cardToken?: string | null, paymentProvider?: string, referralCode?: string, turnstileToken?: string, attribution?: Record<string, unknown>, phone?: string) => {
        try {
            const { data, error: functionError } = await supabase.functions.invoke('signup-handler', {
                body: { email, password, full_name: fullName, clinic_name: clinicName, phone, selected_plan: selectedPlan, card_token: cardToken, payment_provider: paymentProvider, referral_code: referralCode, turnstile_token: turnstileToken, attribution }
            })
            if (functionError) {
