...
interface AuthContextType {
    ...
    signUp: (email: string, password: string, fullName: string, clinicName: string, selectedPlan: string, cardToken?: string | null, paymentProvider?: string, referralCode?: string, turnstileToken?: string, attribution?: Record<string, unknown>, phone?: string, country?: string) => Promise<{ error: Error | null }>
...
export function AuthProvider({ children }: { children: ReactNode }) {
    ...
    const signUp = async (email: string, password: string, fullName: string, clinicName: string, selectedPlan: string, cardToken?: string | null, paymentProvider?: string, referralCode?: string, turnstileToken?: string, attribution?: Record<string, unknown>, phone?: string, country?: string) => {
        try {
            const { data, error: functionError } = await supabase.functions.invoke('signup-handler', {
                body: { email, password, full_name: fullName, clinic_name: clinicName, phone, country, selected_plan: selectedPlan, card_token: cardToken, payment_provider: paymentProvider, referral_code: referralCode, turnstile_token: turnstileToken, attribution }
            })
            if (functionError) {
