                    ...
                    <tbody>
                        {rules.map(r => (
                            <tr key={r.localId} className="border-t border-silk-beige/50">
                                ...
                                <td className="py-1.5 pr-2"><input type="number" value={r.duration_minutes ?? ''} placeholder={String(baseDuration || '')} onChange={e => patch(r.localId, { duration_minutes: e.target.value ? Number(e.target.value) : undefined })} className="input-soft py-1 text-xs w-20" /></td>
