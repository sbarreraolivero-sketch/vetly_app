...
| Variante Meta "a ciegas" (`/core-a-ciegas`) | ✅ **Creada 2026-08-28** — copia de `/core` con message match al video ad `video-ads-1.mp4` (ángulo "Así se ve trabajar a ciegas"). Diff acotado: hero H1/subtítulo, sección "¿Te suena familiar?" (sub-línea "Así se ve trabajar a ciegas, en la práctica" + 3 cards), H2 de cierre, tags SEO/OG. `noindex, follow` + `canonical` → `/core`. Testimonios copiados verbatim (decisión del usuario; siguen marcados como inventados/riesgo Ley del Consumidor). Archivo `public/core-a-ciegas.html`, ruta en `vercel.json`. **Tracking heredado sin código nuevo** (carga `/vetly-tracking.js` + CTA a `/register?plan=core`). Es solo para **tráfico pagado de Meta**, no de Google. |
...
| 2026-08-28 | **Cohorte Core + secuencia de correos de onboarding (fuera del alcance Google).** Snapshot de las clínicas Core: **15 registros en 7 días (21-28 ago)**, todas en prueba, **ninguna pagada**, activación casi nula (9 de 14 reales en cero absoluto — 0 pacientes, 0 ingresos; solo Servando Mendoza importó 236 pacientes). La ola viene de la campaña de **Meta** (los 4 leads de sesión 84 están en ella); Google aún no gasta. Se activó una **secuencia fija de correos de onboarding** (`cron-lifecycle-emails`, cron diario 14:00 UTC, jobid 21): 8 pasos por día mínimo (clínica+horarios+equipo, servicios, inventario, pacientes, finanzas, fidelización, recordatorios) + 2 avisos de fin de prueba, 1 correo/día máx, botón de soporte WhatsApp en cada uno. Es el rescate de esta cohorte estancada. Detalle completo en `CLAUDE.md` sesión 90. Dato pendiente aparte: `clinic_settings.country` viene `null` en todos los registros nuevos — el selector de país del registro no lo persiste. |
| 2026-08-28 | **Landing de video para Meta Ads (fuera del alcance Google).** Creada `public/core-a-ciegas.html` (URL `/core-a-ciegas`) como copia de `/core` con message match al video ad `video-ads-1.mp4`, ángulo **"Así se ve trabajar a ciegas"** — sin sistema, dirigir la clínica es a ciegas (info dispersa: historial, caja, stock). Message match: H1 "Deja de trabajar a ciegas en tu propia clínica", sub-línea de la sección de dolor con el frame clave del video verbatim, H2 de cierre "Deja de trabajar a ciegas. Empieza hoy." Metáfora "a ciegas" acotada a **información/organización**, nunca a resultados clínicos (el video dramatiza una emergencia; Vetly no previene emergencias). Respeta `feedback_core_plan_messaging.md` (ángulo de gestión pura, no de Lía). `noindex,follow` + canonical → `/core` (variante solo-pauta, casi duplicada). **Tracking:** cero código nuevo — hereda `/vetly-tracking.js` + CTA a `/register?plan=core`. La URL del anuncio lleva `utm_campaign=core_a_ciegas` (o basta con `landing_url ilike '%core-a-ciegas%'` + `fbclid`, que Meta siempre agrega). Optimizar el ad set por `CompleteRegistration`. Limitación heredada: pixel client-side, Meta sub-reporta ~1 de 4. Cambio de código: `public/core-a-ciegas.html` (nuevo) + 1 línea en `vercel.json`. |
