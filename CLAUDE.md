...
- **Starter: `plan_limits` `max_users` 5→3, `max_agendas` 3→2** (migración `starter_plan_3_users_2_agendas`) para que el salto a Pro (10 usuarios) sea más atractivo. Mirrors + copy actualizados en los 4 lugares (el bloque de landing decía "2 usuarios · 1 agenda" por drift, ahora "3 usuarios · 2 agendas"). 0 clínicas Starter reales. `invite_member_v2` lee `max_users` de `plan_limits` por `plan_id`, sin CASE hardcodeado.
  - **Decisión (2026-09-02): Core se queda en `max_users = 10`.** "Usuarios" NO es el eje de upgrade Core→Starter — el valor de Starter es el agente IA + Goldi + más agendas + créditos. Para que el card de Starter no mienta ("Todo lo de Core" con menos usuarios), la línea `'Todo lo de Core'` se reformuló a **`'Todas las funciones de gestión de Core'`** en `paddle.ts`, `mercadopago.ts`, `Pricing.tsx` y `landing.html` — deja claro que hereda los módulos/funciones, no los límites. Las líneas "Todo lo de Starter" (Pro) y "Todo lo de Pro" (Enterprise) NO se tocan: ahí el plan hijo sí tiene límites ≥ que el padre.
...
## Cambios realizados — septiembre 2026 (sesión 98, 2026-09-02/03)

Continuación directa de sesión 97 (generalización del modo coordinadora a Linares). El mismo día de la activación aparecieron dos casos reales más — distintos entre sí — que expusieron que la "última barrera" (aviso de la coordinadora sin rastro, sesión 92) no cerraba el problema completo. Terminó con un rediseño de cómo se hace cumplir el flujo de coordinación: de "confiar en el prompt + avisar si falla" a "forzar la corrección en código antes de dejar salir una respuesta rota".

### Caso 1 — "scarlett"/Mila: el aviso a la coordinadora quedó sin rastro (Santiago)

**Reporte de Claudia (vía el usuario, con captura real):** una solicitud de agenda (tutora real: **María Francisca Rodríguez**, Pirque, perrita **Mila** — "scarlett" era solo el nombre guardado en la agenda de contactos del teléfono, sin relación con la conversación) le llegó al sistema — Claudia la vio y la descartó desde el panel varias horas después — pero **nunca le llegó el aviso por WhatsApp a su celular personal**.

**Diagnóstico con datos reales, no supuesto:**
- `scheduling_requests` confirma la fila creada correctamente (`tutor_name`, `pet_name`, `address`, `availability_text` todos con datos reales) y **descartada por Claudia 13,7 horas después** (`reviewed_at` desde el dashboard) — la solicitud sí "le llegó al sistema", tal como Claudia describía.
- **Cero entradas en `debug_logs` con el prefijo `[COORDINATOR ALERT]`** en la ventana del envío. Causa: el logging que hace visible el resultado real del envío (agregado en sesión 92, commit `94dab85`) se desplegó **hora y media después** de que este intento de envío ocurriera — el caso cayó justo en el hueco anterior al fix. No hay forma honesta de reconstruir con datos qué pasó esa noche específica.
- **Prueba en vivo del mecanismo** (función de diagnóstico temporal, desplegada y borrada en el acto): un envío real de prueba a `+56989790949` (número de Claudia) con las credenciales de Santiago se aceptó sin problema (Meta devolvió `200` + `message.id`) — el mecanismo de envío en sí no está roto.

**El hueco real que sí se encontró y se corrigió:** aunque Meta acepte el envío al momento de mandarlo (200 + `message.id`), puede rechazarlo **después**, de forma asíncrona (ej. ventana de 24h vencida) — y hasta ahora ese rechazo posterior no dejaba ningún rastro, ni siquiera con el logging de sesión 92. **Fix** (`meta-whatsapp-webhook`, `requestSchedulingCoordination()`): el aviso a la coordinadora ahora se persiste en `messages` con su `ycloud_message_id` (el WAMID real que devuelve Meta) — insert **directo**, no vía `saveMsg()`, para no disparar el cobro de créditos IA de un mensaje que nunca fue una respuesta de OpenAI. Con el WAMID guardado, el handler de `whatsapp.message.updated` **que ya existe en el mismo archivo** (usado para recordatorios desde sesión 56) correlaciona automáticamente cualquier `delivered`/`read`/`failed` posterior — sin necesidad de código nuevo de status-tracking, solo de que el mensaje quedara identificable. Commit `ab675aa`.

### Caso 2 — Vanesa Torres/Cuki: la IA prometió coordinar y nunca llamó a la función (Linares)

**Reporte con captura real, mismo día:** la IA recolectó todos los datos correctamente (tutor, mascota, síntomas, disponibilidad, ubicación) y respondió *"Voy a enviar esta información a nuestra coordinadora para que revise la ruta..."* — pero **nunca ejecutó `request_scheduling_coordination`**.

**Confirmado con evidencia, no asumido:** `messages.ai_function_called` de esa respuesta es **`NULL`**, y no existe ninguna fila en `scheduling_requests` para ese teléfono. El mensaje de "voy a coordinar" fue, literalmente, una promesa vacía — la solicitud jamás se creó, nadie la vio, nadie la iba a poder autorizar nunca.

**Primer fix (detección post-envío, commit `8835760`):** justo después de enviar la respuesta al tutor, se compara el texto contra un patrón ("coordinador", "revisará la ruta", "voy a enviar...") y contra si efectivamente se ejecutó algún tool de cierre (`request_scheduling_coordination`/`create_appointment`/`reschedule_appointment`/`escalate_to_human`). Si hay promesa sin acción: pausa al tutor (`requires_human=true`) y crea una notificación in-app de tipo `human_handoff` para que alguien revise manualmente.

**El usuario pidió reforzarlo más — "esto se refuerce con una regla jacodeada, porque la verdad que está fallando constantemente este error".** La detección post-envío solo avisaba *después* de que el tutor ya hubiera recibido la promesa rota — no lo prevenía. La misma prohibición ("TERMINANTEMENTE PROHIBIDO decir 'un momento' sin ejecutar la función") ya vivía casi palabra por palabra en `ai_behavior_rules`, y el modelo la violó igual — reforzar solo el prompt de nuevo no era confiable.

**Fix estructural (commit `aad5d66`):** se restructuró el tool loop de `meta-whatsapp-webhook` (el mismo `while` que procesa las llamadas a funciones) para que, cuando el modelo responde solo texto con la promesa de coordinar **sin** haber llamado a ningún tool de cierre, **esa respuesta no se acepte como final**. En su lugar:
1. Se empuja la respuesta rota del modelo + un mensaje de `role: "system"` forzando la corrección: *"...llama AHORA MISMO a request_scheduling_coordination con esos datos exactos. Si te falta alguno, NO repitas esa frase: pregúntaselo directamente al tutor."*
2. Se vuelve a llamar a OpenAI (mismo bucle, sin gastar un tool-call-budget aparte) — el modelo puede: (a) llamar al tool correctamente si ya tenía todos los datos, o (b) hacer una pregunta real por lo que falte, en vez de repetir la frase vacía.
3. Bloqueado a **un solo reintento** (`correctionAttempts < 1`) — no reabre un loop infinito ni fuerza `tool_choice` a una función específica (forzar el tool a ciegas habría arriesgado crear una solicitud con datos incompletos/alucinados, ej. especie/sexo de la mascota, que no está entre los campos con guard anti-placeholder).
4. La detección post-envío de `8835760` se mantiene como **último respaldo** — reutiliza las mismas constantes (`dispatchTools`/`promisePattern`) que ahora viven hoisted al inicio del bloque del tool loop, en vez de estar duplicadas — para el caso raro en que ni siquiera el reintento forzado lo corrija.

**Deploy:** `meta-whatsapp-webhook` (2 deploys — el de la detección y el de la corrección forzada), `verify_jwt: false` preservado. Verificado sin errores nuevos en `debug_logs` tras cada deploy con tráfico real entrando. **No verificado todavía con un caso real de promesa-sin-acción posterior al deploy** — la próxima vez que ocurra debería quedar visible en `debug_logs` como `[COORDINATION PROMISE GAP] Forzando reintento con corrección` (si el reintento lo arregla) o `...Persistió incluso después del reintento forzado` (si no).
...
#### Verificación visual real — confirmada por el usuario en producción (mismo día)

El usuario abrió `vetly.pro/app/settings` con la cuenta real de Claudia y confirmó visualmente la tab "Servicios y Precios" para **ambas** clínicas — cierra el punto pendiente de arriba, la UI renderiza y funciona en producción real, sin ninguno de los problemas de auth/timing que bloquearon la verificación por Playwright en este entorno.

Un detalle que el usuario preguntó al ver la matriz de Linares ("¿Creo que faltó sobre la anestesia inhalatoria?") resultó ser correcto por diseño, no un bug — verificado contra el documento KB real (`MATRIZ_PRECIOS_Y_PROTOCOLO_CIRUGIAS`) antes de responder, no de memoria:
- **Linares/Talca no ofrece anestesia inhalatoria en absoluto** — su propio protocolo excluye explícitamente razas braquicéfalas ("requieren anestesia inhalatoria, procedimiento que no realizamos en ruta"). Por eso su matriz de cirugía varía por **tramo de traslado (T1/T2/T3)**, no por tipo de anestesia — coincide exactamente con lo mostrado en pantalla (especie/sexo/peso/T1/T2/T3 + recargo celo-preñez).
- **Santiago sí ofrece ambos tipos** (inyectable + inhalatoria, obligatoria para braquicéfalas) — su matriz usa `anesthesia_type` en vez de tramo (el recargo ahí es por comuna). El usuario confirmó que esta matriz también está completa.

### Pendiente (según el plan original de la matriz de precios)

- [x] Verificación visual real de la tab "Servicios y Precios" — confirmada por el usuario en producción, ambas clínicas, el mismo día del deploy.
- [ ] Recrear los 4 casos reales desde la interfaz y comparar contra las filas cargadas por SQL (deben ser idénticas) — paso 5 del plan, ya no bloqueado por lo de arriba pero no se hizo la comparación fila-por-fila explícita.
...
### Barrido post-deploy — 2 documentos con instrucción de "calcular a mano" que sobrevivieron a la migración

El usuario preguntó, tras confirmar visualmente la UI, si debía limpiarse la KB que antes tenía los valores crudos de cirugía/destartraje. Verificado contra el contenido real (no de memoria): los 4 documentos KB principales (`MATRIZ_PRECIOS_Y_PROTOCOLO_CIRUGIAS` y `Protocolo_de_Destartraje`, ambas clínicas) **ya estaban limpios** desde el trabajo previo de esta misma sesión — sin tabla cruda, apuntando a `calculate_matrix_price`. Pero el barrido encontró **2 lugares reales que sí seguían con la instrucción vieja de "lee la tabla y calcula"**, ambos anteriores a la existencia del tool y nunca actualizados:

1. **`ai_behavior_rules` de Santiago, sección 6 PASO 4 (cirugías)** — el más grave: decía literalmente *"usa la tabla '2. PROCEDIMIENTOS CON ANESTESIA INHALATORIA'... usa la tabla de anestesia Inyectable... el precio varía por peso Y por tipo de anestesia **según la tabla del KB**"*, con un ejemplo de precio concreto embebido ("gato macho = $70.000"). Como `ai_behavior_rules` va siempre en el prompt (a diferencia del KB, que solo se fuerza por keyword), esta instrucción competía directamente con la del propio KB (que sí decía "prohibido calcular"). Reescrito: mantiene la lógica legítima de determinar `anesthesia_type` por raza braquicéfala (eso sigue siendo un dato de entrada necesario para la función), pero reemplaza "usa la tabla X/Y" por "pásale `anesthesia_type` a la función" y elimina el ejemplo de precio concreto.
2. **KB `PROTOCOLO_LOGISTICA_CIRUGIAS_ANIMALGRACE` (solo Linares)** — Sección 2 tenía la tabla cruda de recargos T1/T2/T3 ($0/$8.000/$16.000) y la Sección 3 decía explícitamente *"Fórmula interna: Precio base... + Recargo del tramo quirúrgico = Precio Total"* — instrucción directa de sumar a mano, sin mencionar la función en absoluto. Reescrito: el recargo de tramo ya está integrado en `calculate_matrix_price` (recibe `travel_minutes`, devuelve el `price_total` ya con el tramo incluido); si la distancia excede el rango, la propia función indica escalar — no hace falta que el documento liste los cortes de minutos como algo para sumar manualmente.
3. **Limpieza menor** — `ai_behavior_rules` de Linares todavía tenía el ejemplo "GATA hembra T1 = \$65.000 (NO \$80.000). GATO macho T1 = \$60.000 (NO \$70.000)" — el guardarraíl anti-confusión de la sesión 40, obsoleto ahora que el cálculo es determinístico vía función. Eliminado (regla permanente de sesiones 9/40: nunca dejar montos concretos como ejemplo en un prompt de precios).

**Verificado con evidencia, no solo con el fix aplicado:**
- Barrido de `information_schema`-style regex sobre ambos `ai_behavior_rules` completos: los 6-7 restantes usos de la palabra "tabla" en cada uno son legítimos y no relacionados (estimación de peso por raza, recargos por comuna de servicios generales, protocolo de consulta multi-mascota — ninguno es un lookup de precio de cirugía/destartraje).
- Los 2 documentos `MATRIZ_PRECIOS_Y_PROTOCOLO_CIRUGIAS` sí contienen `|` y `$` en el mismo documento (dieron falso positivo en un primer chequeo con regex), pero verificado línea por línea: no hay ninguna fila de tabla markdown con precios — son menciones sueltas (piometra/cesárea, marcadas explícitamente "fuera de la matriz — mantener como texto"; examen prequirúrgico) en párrafos distintos a cualquier tabla.
- Contenido final de ambos documentos re-leído completo tras el fix — ambos siguen legibles de punta a punta, sin frases colgando ni contradicciones internas.
...
1. Creada la plantilla **`nueva_solicitud_agenda` como `UTILITY`** en ambas WABAs (Linares `1039327445154499`, Santiago `903775156940145`), vía Graph API con el `meta_access_token` de cada clínica. Mismo body/5 params que la anterior, pero texto reescrito para leerse claramente transaccional: **sin emoji al inicio, sin formato de titular con guión largo, frase completa con punto final** ("Nueva solicitud de agenda para revisar la ruta." vs la vieja "🐾 Nueva solicitud de agenda — revisar ruta"). El clasificador de Meta la aprobó como `UTILITY` (a la vieja la había recategorizado a `MARKETING` en la creación, probablemente por el emoji + estilo titular). Meta la aprobó en minutos.
...
**Reglas permanentes:**
- **Toda plantilla de WhatsApp que sea una notificación operativa/transaccional (avisos internos, confirmaciones, alertas de estado) debe crearse como `UTILITY`, nunca `MARKETING`.** Las `MARKETING` están sujetas al frequency cap `131049` de Meta, que se acumula por número destinatario across-businesses y puede llegar a bloquear el 100% de los envíos a un mismo número.
- **`allow_category_change` ya no existe (Meta lo eliminó el 9-abr-2025).** El clasificador de Meta decide la categoría por el **contenido**, sin importar la que pidas — y puede **recategorizar una `UTILITY` ya aprobada a `MARKETING`** en su revisión continua. Para que se mantenga UTILITY: texto plano transaccional, sin emojis al inicio, sin CTAs promocionales, sin formato de titular. Si Meta la recategoriza, se puede **pedir revisión dentro de los 60 días** (Business Manager → la plantilla → solicitar revisión de categoría). Conviene chequear periódicamente la `category` real de `nueva_solicitud_agenda` en ambas WABAs.
