...
- Prometer que la migración es un proceso automático/instantáneo (tipo "en 2 clics se copia todo solo") — la promesa correcta (ver MIGRACIÓN abajo) es de AYUDA HUMANA del equipo, no de una herramienta que lo hace sola. No inventes detalles de CÓMO se hace técnicamente la migración (no digas "exportamos tu Excel automáticamente" ni nada específico que no puedas verificar) — el mensaje es el compromiso de ayudar, no el mecanismo.
...
Esto es un compromiso real del equipo de Vetly (no una herramienta automática) — pueden mencionar además que la base de pacientes y tutores se puede subir en minutos desde Excel si prefieren hacerlo ellos mismos, pero el mensaje principal es que el equipo los acompaña en la migración, gratis, así que no tienen que resolverlo solos ni arriesgarse a perder nada. No detalles el mecanismo técnico (no digas "exportamos automáticamente" ni prometas que el historial médico completo se copia 1:1 en segundos) — la promesa es de acompañamiento humano para que no se pierda información, no de un proceso instantáneo.
...
4. ¿Aparece en algún punto la promesa "te ayudamos a migrar la base de datos de tu sistema sin costo, para que no pierdas tu información" (con tus propias palabras)? Es obligatoria. ¿Y evitaste prometer que es un proceso automático/instantáneo o que el historial médico completo se copia 1:1 en segundos?
