-- Hallazgo de la revisión de seguridad de esta sesión: la rama del bono de
-- BIENVENIDA nunca validó que el referidor no fuera el propio comprador (a
-- diferencia de la rama del bono AL REFERIDOR, que sí lo hace con
-- `v_referrer <> v_income.tutor_id`). El buscador manual de NewIncomeForm.tsx
-- ya excluye auto-selección (`t.id !== selectedTutor?.id`), pero el canal de
-- WhatsApp detecta el código de referido por regex sin ese chequeo — un
-- cliente podía escribir su propio código y cobrar el bono de bienvenida (que
-- de otro modo no aplica sin referidor). Antes era de bajo impacto (tope
-- fijo de 200); esta sesión lo volvió 15% sin techo como preset para toda
-- clínica nueva, así que se cierra ahora. Único cambio funcional: la
-- condición del bloque welcome_bonus ahora también exige que el referidor no
-- sea el propio tutor, igual que ya exige la rama referral_reward.
...
BEGIN
    ...
    IF v_is_first AND NOT v_had_welcome THEN
        ...
        -- NUEVO: un referidor igual al propio comprador no cuenta como
        -- referido real — sin esto, autorreferirse (posible vía el código
        -- detectado por WhatsApp) desbloqueaba el bono de bienvenida.
        IF v_referrer = v_income.tutor_id THEN
            v_referrer := NULL;
        END IF;
