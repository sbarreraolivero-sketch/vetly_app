-- Firma digital del profesional, para estampar en las recetas (y otros
-- documentos a futuro). Es por-profesional, así que vive en clinic_members.
-- La imagen se sube al bucket público `clinic-branding` (mismo que el logo),
-- path `{clinic_id}/signatures/{member_id}.png` — la política existente ya
-- da escritura al miembro de la clínica y lectura pública (la receta pública
-- /receta/:token necesita mostrarla).
--
-- prescriptions.prescriber_signature_url = snapshot al emitir: la receta
-- histórica conserva la firma que estaba vigente en ese momento aunque el
-- profesional la cambie después (igual criterio que prescriber_name/license).

ALTER TABLE public.clinic_members
    ADD COLUMN IF NOT EXISTS signature_url TEXT;

COMMENT ON COLUMN public.clinic_members.signature_url IS
    'URL pública de la imagen de firma del profesional (bucket clinic-branding). Se estampa en recetas y documentos.';

ALTER TABLE public.prescriptions
    ADD COLUMN IF NOT EXISTS prescriber_signature_url TEXT;

-- get_prescription_public: agregar la firma al bloque `prescription`.
-- CREATE OR REPLACE (mismo tipo de retorno JSONB, solo una clave más).
