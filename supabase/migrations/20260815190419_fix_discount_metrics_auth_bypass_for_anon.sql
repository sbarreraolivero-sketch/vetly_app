-- ============================================================================
-- get_finance_discount_metrics tenía el mismo patrón de bypass que las RPCs de
-- ingresos: `auth.uid() IS NOT NULL` se salta entero cuando auth.uid() es NULL,
-- que es el caso tanto de service_role COMO de anon.
--
-- No era explotable porque el GRANT no incluía a anon, pero dejarlo así hace que
-- la seguridad dependa de un GRANT y no de la lógica. Se armoniza con auth.role(),
-- igual que create/update_clinic_income.
--
-- El cuerpo de la consulta queda idéntico; solo cambia el guardia y se fija
-- search_path (la función no lo declaraba).
-- ============================================================================

CREATE OR REPLACE FUNCTION public.get_finance_discount_metrics(
    p_clinic_id uuid, p_start timestamptz, p_end timestamptz
)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
...
REVOKE EXECUTE ON FUNCTION public.get_finance_discount_metrics(uuid, timestamptz, timestamptz) FROM anon;
