-- Migration: fix_scheduling_lunch_and_intervals
-- Description: Ensures global and professional availability respects lunch breaks and uses clean service-duration intervals.

-- 1. Fix get_available_slots (Global)
...
BEGIN
  ...
  WHILE v_current_time + (p_duration || ' minutes')::INTERVAL <= v_close_time LOOP
    ...
    -- check_availability internally checks clinic lunch break
...
-- 2. Fix get_professional_available_slots (Professional)
...
BEGIN
  ...
  -- IMPORTANT: Intersect with CLINIC Lunch Break
  ...
  WHILE v_slot_start + (p_duration || ' minutes')::INTERVAL <= v_effective_end LOOP
    ...
    IF v_is_free THEN
      ...
      is_available := TRUE;
      RETURN NEXT;
      v_slot_start := v_slot_start + (p_duration || ' minutes')::INTERVAL;
    ELSE
